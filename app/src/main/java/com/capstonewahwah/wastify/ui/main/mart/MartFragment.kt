...
import com.capstonewahwah.wastify.databinding.FragmentMartBinding
...
class MartFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val martViewModel =
            ViewModelProvider(this).get(MartViewModel::class.java)

        _binding = FragmentMartBinding.inflate(inflater, container, false)
        val root: View = binding.root

        val textView: TextView = binding.textDashboard
        martViewModel.text.observe(viewLifecycleOwner) {
            textView.text = it
        }
        return root
    }
